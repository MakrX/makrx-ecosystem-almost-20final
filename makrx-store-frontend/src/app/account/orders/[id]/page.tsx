      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-2 text-sm text-store-text-muted mb-4">
            ...
            <ArrowRight className="h-3 w-3" />
            <Link href="/account/orders" className="hover:text-store-primary">Orders</Link>
            <ArrowRight className="h-3 w-3" />
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <h1 className="text-3xl font-bold text-store-text">Order {order.number}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-xl font-semibold text-store-text mb-6">Order Timeline</h2>
                    ...
                    <div key={index} className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <h3 className={`font-medium ${event.completed ? 'text-store-text' : 'text-gray-400'}`}>
                            {event.status}
                          </h3>
                          <span className={`text-sm ${event.completed ? 'text-store-text-muted' : 'text-gray-400'}`}>
                            {formatDate(event.date)}
                          </span>
                        </div>
                        <p className={`text-sm mt-1 ${event.completed ? 'text-store-text-muted' : 'text-gray-400'}`}>
                          {event.description}
                        </p>
                      </div>
                    </div>
              ...
              {order.shipping.trackingNumber && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="font-medium text-store-text mb-2">Tracking Details</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-store-text-muted">Carrier:</span>
                          <span className="text-store-text">{order.shipping.carrier}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-store-text-muted">Method:</span>
                          <span className="text-store-text">{order.shipping.method}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-store-text-muted">Tracking Number:</span>
                          <div className="flex items-center gap-2">
                            <span className="text-store-text font-mono">{order.shipping.trackingNumber}</span>
                            <button
                    ...
                    <div>
                      <h3 className="font-medium text-store-text mb-2">Delivery Information</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-store-text-muted">Estimated Delivery:</span>
                          <span className="text-store-text">{formatDate(order.estimatedDelivery)}</span>
                        </div>
                        {order.actualDelivery && (
                          <div className="flex justify-between">
                            <span className="text-store-text-muted">Actual Delivery:</span>
                            <span className="text-store-text">{formatDate(order.actualDelivery)}</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {/* Order Items */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-xl font-semibold text-store-text mb-6">Order Items</h2>
                <div className="space-y-4">
                  {order.items.map((item) => (
                    <div key={item.id} className="flex items-center gap-4 p-4 border border-gray-200 rounded-lg">
                      ...
                      />
                      <div className="flex-1">
                        <h3 className="font-medium text-store-text mb-1">{item.product.name}</h3>
                        <p className="text-sm text-store-text-muted mb-2">
                          SKU: {item.product.sku}
                        </p>
                        <div className="flex items-center gap-4 text-sm">
                          <span className="text-store-text-muted">Qty: {item.quantity}</span>
                          <span className="text-store-text-muted">Price: ${item.price.toFixed(2)} each</span>
              ...
              {order.notes && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <h2 className="text-xl font-semibold text-store-text mb-4">Special Instructions</h2>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex justify-between">
                      <span className="font-semibold text-store-text">Total</span>
                      <span className="font-bold text-store-text">${order.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-store-text">{order.customer.email}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Phone className="h-4 w-4 text-store-text-muted" />
                    <span className="text-store-text">{order.customer.phone}</span>
                  </div>
                </div>
              </div>
