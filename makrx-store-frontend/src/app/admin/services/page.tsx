...
import { Search, Filter, Download, Eye, Printer, Play, Pause, Check, X, Clock, Settings, FileText, AlertTriangle, ArrowUpDown } from 'lucide-react';
...
interface ServiceJob {
  ...
  material: string;
  quantity: number;
  status: 'queued' | 'printing' | 'post-processing' | 'quality-check' | 'completed' | 'failed' | 'cancelled';
...
export default function AdminServices() {
  ...
  const getStatusIcon = (status: ServiceJob['status']) => {
    switch (status) {
      ...
      case 'post-processing': return <Settings className="w-4 h-4" />;
      case 'quality-check': return <Eye className="w-4 h-4" />;
  ...
  const getStatusColor = (status: ServiceJob['status']) => {
    switch (status) {
      ...
      case 'post-processing': return 'bg-yellow-100 text-yellow-800';
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {mockServiceJobs.filter(j => ['printing', 'post-processing', 'quality-check'].includes(j.status)).length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  ${mockServiceJobs.filter(j => j.status === 'completed').reduce((sum, j) => sum + j.price, 0).toFixed(2)}
                </p>
              </div>
            </div>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full capitalize ${getPriorityColor(job.priority)}`}>
                        {job.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
