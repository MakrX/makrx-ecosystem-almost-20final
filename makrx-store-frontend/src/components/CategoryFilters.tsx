...
export default function CategoryFilters({ facets, activeFilters, onFilterChange }: CategoryFiltersProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        {facets.map((facet, index) => (
          <div key={facet.name} className="border-b border-gray-200 dark:border-gray-700 last:border-b-0">
            <button
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {facet.values?.map((value: any) => (
                    <label key={value.name || value.id} className="flex items-center">
                      <input
                        type="checkbox"
                        checked={activeFilters[facet.name]?.includes(value.name || value.id) || false}
                        onChange={(e) => {
