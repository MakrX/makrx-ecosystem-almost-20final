...
export default function SearchFilters({ facets, activeFilters, onFilterChange }: SearchFiltersProps) {
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['brands', 'material', 'price']));
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
        ...
        {facets.map((facet) => (
          <div key={facet.name} className="border-b border-gray-200 dark:border-gray-700 last:border-b-0 py-4 last:pb-0">
            <button
            ...
            {expandedSections.has(facet.name) && (
              <div className="mt-3 space-y-2">
                {facet.type === 'checkbox' && facet.values.map((value) => (
                  <label key={value.name} className="flex items-center">
                    <input
                      type="checkbox"
                      checked={activeFilters[facet.name]?.includes(value.name) || false}
                      onChange={(e) => handleFilterChange(facet.name, value.name, e.target.checked)}
