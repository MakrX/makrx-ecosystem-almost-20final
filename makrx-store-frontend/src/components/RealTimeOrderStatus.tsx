...
export default function RealTimeOrderStatus({ orderId, currentStatus }: OrderStatusProps) {
  const [status, setStatus] = useState(currentStatus)
  const [statusHistory, setStatusHistory] = useState<Array<{ status: string, timestamp: Date, note?: string }>>([])
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (update.status !== status) {
              ...
              const statusStep = statusSteps.find(step => step.key === update.status)
              addNotification({
                type: 'info',
          ...
          <div className="space-y-2">
            {statusHistory.slice(-3).reverse().map((entry, index) => (
              <div key={index} className="flex justify-between text-xs text-gray-600">
                <span>{entry.note || `Status updated to ${entry.status}`}</span>
