...
const CrossPortalAuthContext = createContext<CrossPortalAuthContextValue | undefined>(undefined)
...
export function CrossPortalAuthProvider({ children }: { children: React.ReactNode }) {
  const { user, token, isAuthenticated } = useAuth()
  const [portalTokens, setPortalTokens] = useState<Map<string, PortalToken>>(new Map())
  ...
  const generatePortalToken = async (portal: string): Promise<PortalToken | null> => {
  ...
  const navigateToPortal = async (portal: string, path: string = '/'): Promise<void> => {
    if (!isAuthenticated) {
...
export function useCrossPortalAuth() {
  ...
  if (context === undefined) {
    throw new Error('useCrossPortalAuth must be used within a CrossPortalAuthProvider')
  }
  return context
}
...
export function usePortalToken(portal: string) {
  const { portalTokens, generatePortalToken, refreshPortalToken } = useCrossPortalAuth()
